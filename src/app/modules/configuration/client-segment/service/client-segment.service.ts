...
export class ClientSegmentService {
   ...
   isLoading$: Observable<boolean>;
    ...
    console.log('RegisterClientSegment', data);
  ...
  listClientSegment(page =1 , search:string = '') {
      console.log('Token de Listclient_segments', this.authservice.token);
      this.isLoadingSubject.next(true);
  ...
  /*
    ...
    updateclient_segments(ID_CLIENT_SEGMENT: number, data: any) {
    console.log('=== INICIO UPDATE ClientSegment ===');
    console.log('ID_ROLE:', ID_CLIENT_SEGMENT);
    ...
    if (!ID_CLIENT_SEGMENT || ID_CLIENT_SEGMENT <= 0) {
      console.error('ID_CLIENT_SEGMENT inválido:', ID_CLIENT_SEGMENT);
      this.isLoadingSubject.next(false);
